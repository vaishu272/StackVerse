...
export function proxy(request: NextRequest) {
  ...
  if (hasRefreshToken) {
    // If we have the role, we enforce strict dashboard gating and prevent auth page access
    if (userRole) {
      ...
      // If userRole cookie is missing but they have refreshToken, we let it proceed to
      // let client-side AuthProvider handle the refresh and set the userRole cookie.
    }
  } 
