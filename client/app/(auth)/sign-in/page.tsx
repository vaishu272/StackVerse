...
export default function LoginPage() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-800 dark:text-slate-100 flex items-center justify-center p-4 md:p-10 relative overflow-hidden theme-transition">
      ...
      <div className="absolute bottom-1/4 right-1/4 w-96 h-96 bg-pink-600/5 dark:bg-pink-600/10 rounded-full blur-3xl -z-10 animate-pulse delay-700"></div>
        ...
        <div className="hidden md:flex md:col-span-5 lg:col-span-6 relative flex-col justify-between p-12 overflow-hidden bg-slate-950 text-white">
          ...
          <div className="relative z-20 mt-auto pt-20">
            <h3 className="text-3xl font-black tracking-tight leading-tight bg-linear-to-r from-white via-indigo-100 to-slate-300 bg-clip-text text-transparent mb-4">
              Explore Advanced System Architectures.
            </h3>
            <p className="text-slate-400 text-sm leading-relaxed max-w-md">
              Sign in to document platforms, analyze engineering design docs,
              and sync with fellow developers in the tech community.
            </p>
          </div>
        </div>

        {/* Right Column - Form Container */}
        <div className="col-span-1 md:col-span-7 lg:col-span-6 p-8 sm:p-12 flex flex-col justify-center">
          <div className="text-center mb-8">
            ...
            <p className="text-slate-500 dark:text-slate-400 text-sm mt-2">
              Sign in to access your dashboard and write-ups
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
            <div>
              ...
              <input
                ...
                {...register("email")}
              />
              <p className="min-h-5 text-rose-500 text-xs font-semibold mt-1">
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                ...
                <Link
                  ...
                  className="text-xs text-indigo-655 dark:text-indigo-400 hover:text-indigo-500 dark:hover:text-indigo-300 font-semibold"
                >
                  Forgot Password?
                </Link>
              </div>
                ...
                <input
                  ...
                  className={`w-full bg-slate-100/50 dark:bg-slate-950/80 border ${
                    errors.password
                      ? "border-rose-500 focus:border-rose-500 focus:ring-rose-500/20"
                      : "border-slate-200 dark:border-slate-800 focus:border-indigo-500 focus:ring-indigo-500/20"
                  } rounded-lg pl-4 pr-12 py-3 text-slate-850 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-600 text-sm transition-all focus:outline-none focus:ring-2`}
                  {...register("password")}
                />
                <button
                  type="button"
                  onClick={() => setShowPassword(!showPassword)}
                  className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-650 dark:hover:text-slate-200 focus:outline-none transition-colors"
                >
